...
def model(vocab_size: int, num_layers: int, units: int, embedding_dim: int, num_heads: int,
    ...
    # features_drop = tf.keras.layers.Dropout(rate=0.5, dtype=d_type)(features)
    # features_outputs = tf.keras.layers.Dense(units=embedding_dim, activation="relu")(features_drop)

    features_outputs = tf.keras.layers.Dense(units=embedding_dim, activation="relu")(features)
    features_drop = tf.keras.layers.Dropout(rate=0.5, dtype=d_type)(features_outputs)


    # outputs_drop = tf.keras.layers.Dropout(rate=0.5, dtype=d_type)(features_outputs)
    # outputs = tf.keras.layers.Dense(units=2, activation="relu")(outputs_drop)

    outputs = tf.keras.layers.Dense(units=2, activation="relu")(features_drop)
    outputs_drop = tf.keras.layers.Dropout(rate=0.5, dtype=d_type)(outputs)


    outputs1 = tf.nn.softmax(logits=outputs_drop, axis=-1)

    return tf.keras.Model(inputs=[premise_inputs, hypothesis_inputs], outputs=outputs1, name=name)
